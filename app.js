...
function saveKB(list){
  localStorage.setItem(KB_STORAGE_KEY, JSON.stringify(list));
}
function catInfo(id){
...
function openKbDrawer(entry){
  ...
  content.querySelector('#kb-delete-btn').onclick = ()=>{
    if(confirm('Excluir esta informação da base de conhecimento?')){
      const list = loadKB().filter(e=>e.id !== entry.id);
      saveKB(list);
      closeDrawer();
      renderKbBoard();
      fillStats();
...
document.getElementById('kb-import-file').addEventListener('change', e=>{
  ...
  reader.onload = ()=>{
    try{
      ...
      const current = loadKB();
      const merged = [...imported, ...current].filter((e,i,arr)=>
